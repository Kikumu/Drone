...
class DroneEnv(gym.Env):
    ...
    def _compute_reward(self, obs, action):
        ...
        overshoot_penalty = -0.5 * (np.exp(1.5 * height_excess) - 1.0)
        ...
        #tilt_penalty = -1.0 * float(tilt)
        survival_bonus= 0.25

        airtime_bonus = np.clip(pos[2] / self.target_pos[2], 0.0, 1.0) * 1.0
...
#6. spin_penalty    tiny       ← almost negligible
    ...
    def _is_terminated(self, obs):
        ...
        rpy = obs[6:9]
        crashed   = pos[2] < 0.05
        flipped   = abs(rpy[0]) > 1.0 or abs(rpy[1]) > 1.0
        out_of_bounds = np.any(np.abs(pos[:2]) > 1.5)
